...
class SerieABayesModelDraw:
    ...
    def test_model(self, test_data):
        """
        Testa l'accuratezza del modello su un set di dati di test.

        :param test_data: Lista di dizionari con chiavi 'home_team', 'away_team', 'result'
        :return: Dictionary con metriche di valutazione
        """
        y_true = []
        y_pred = []
        ...
        for match in test_data:
            ...
            y_pred.append([prediction['home_win'], prediction['draw'], prediction['away_win']])
        ...
        brier_score = brier_score_loss(np.array(y_true).ravel(), np.array(y_pred).ravel())

        accuracy = sum(np.argmax(yt) == np.argmax(yp) for yt, yp in zip(y_true, y_pred)) / len(y_true)
